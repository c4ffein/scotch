...
/*
** Property: ordering a weighted graph (with velotab) produces
** a valid permutation and passes orderCheck.
*/
static
void
testOrderWeighted (
hegel_testcase *            tc)
{
  ...
  HEGEL_ASSERT (SCOTCH_graphOrder (&grafdat, &stratdat,
                                   ...
                                   NULL, NULL) == 0,
                "graphOrder failed on weighted graph");
  ...
  HEGEL_ASSERT (SCOTCH_graphOrderCheck (&grafdat, &ordedat) == 0,
                "graphOrderCheck failed on weighted graph");
...
char *              argv[])
{
  ...
  printf ("Running weighted graph ordering test...\n");
  hegel_run_test (testOrderWeighted);
  printf ("PASSED\n");
